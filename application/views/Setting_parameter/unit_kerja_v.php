...
<script>
    $(document).ready(function() {
        ...
        function default_dt() {
            $('#manage_menu').DataTable({
                ...
                "ajax":
                {
                    "url": "<?= base_url('Setting_parameter/unit_kerja_c/get/');?>", // URL file untuk proses select datanya
                    "type": "POST"
                },
                "deferRender": true,
                ...
                "columns": [
                    ...
                    {data: null,
                        render: function (data, type, row, meta) {
                            return '<button class="btn btn-success m-3" onclick="edit_modal()" value="'+data.kode_unit+'">Edit</button> <button class="btn btn-danger" onclick="delete_modal()"  value="'+data.kode_unit+'">Delete</button>';
                        }
                    } 
                ],
            });
    ...
    $("#add").on('click',()=>{
        var val = {};
        val.modal = 'MODAL ADD UNIT KERJA';
        val.id = 'modal_add';
        $.ajax({    
            url:'<?= base_url('Setting_parameter/unit_kerja_c/modal')?>',
            type:"post",
            data: val,
    ...
    function edit_modal(){
        var val = {};
        val.modal = 'MODAL EDIT UNIT KERJA';
        val.id = 'modal_edit';
        val.kode_unit = event.target.value;
    ...
    function delete_modal(){
        var val = {};
        val.modal = 'MODAL DELETE UNIT KERJA';
        val.id = 'modal_delete';
        val.kode_unit = event.target.value;
    ...
    function edit(form){
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url:'<?= base_url('Setting_parameter/unit_kerja_c/edit_/')?>',
                    type:"post",
                    data: form,
                    success: function (res){
    ...
    function save(form){
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url:'<?= base_url('Setting_parameter/unit_kerja_c/save_/')?>',
                    type:"post",
                    data: form,
                    success: function (res){
    ...
    function delete_(form){
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url:'<?= base_url('Setting_parameter/unit_kerja_c/delete_/')?>',
                    type:"post",
                    data: form,
                    success: function (res){
    ...
    function key(tes){
        ...
        $.ajax({
            type: "POST",
            url: "<?php echo site_url('Setting_parameter/unit_kerja_c/validate_keyup/');?>", 
            data: val,
            dataType: "json",  
            success: function(data){
                $.each(data, function(key, value) {
