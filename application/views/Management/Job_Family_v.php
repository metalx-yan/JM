...
<script>
    ...
    function default_dt() {
            $('#manage_menu').DataTable({
                ...
                "ajax": {
                    "url": "<?= base_url('Management/Job_Family_c/get/'); ?>", // URL file untuk proses select datanya
                    "type": "POST"
                },
                "deferRender": true,
                "aLengthMenu": [
    ...
    function action_submit(data_) {
        ...
        if (action == 'modal_delete') {
            ...
            $.ajax({
                type: "POST",
                url: "<?php echo site_url('Management/Job_Family_c/validate'); ?>",
                data: form,
                dataType: "json",
    ...
    $("#add").on('click', () => {
        var val = {};
        val.modal = 'MODAL JOB FAMILY';
        val.id = 'modal_add';
        console.log(val);
        $.ajax({
            url: '<?php echo base_url('Management/job_family_c/modal') ?>',
            type: "post",
            data: val,
    ...
    function edit_modal() {
        var val = {};
        val.modal = 'MODAL EDIT JOB FAMILY';
        val.id = 'modal_edit';
        val.form_id = "form_" + val.id;
        val.id_key = event.target.value;
        console.log(val);
        $.ajax({
            url: '<?= base_url('Management/Job_family_c/modal/') ?>',
            type: "post",
            data: val,
            success: function(res) {
                // alert(res);
    ...
    function save(form) {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: '<?= base_url('Management/Job_Family_c/save_/') ?>',
                    type: "post",
                    data: form,
                    beforeSend: function() {
                        $("#loader").show();
    ...
    function edit(form) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: '<?= base_url('Management/Job_Family_c/edit_/') ?>',
                    type: "post",
                    data: form,
                    success: function(res) {
